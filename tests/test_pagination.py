...
@unittest.skipIf(
    not factory, "Must have Django REST Framework installed to run pagination tests."
)
class TestSequenceCursorPagination(TestCase):
    ...
    def setUp(self):
        ...
        self.queryset = QuerySetSequence(
            Book.objects.filter(pages__lte=7), Book.objects.filter(pages__gt=7)
        )
    ...
    def test_multiple_ordering(self):
        ...
        pages = [
            b.pages for b in self.queryset.order_by("#", *self.pagination.ordering)
        ]
    ...
    def test_duplicates(self):
        ...
        for i in range(3):
            ...
            self.assertEqual(
                titles, tuple(map(lambda d: str(d + (i * 5)), [0, 1, 2, 3, 4]))
            )
